...
- `ctx.workspaceRegistry.unarchiveSession(id)` — removes one session from the archive set durably, restoring it to every grouping surface; an id outside the set resolves without writing.
- `ctx.workspaceRegistry.deleteSession(id)` — permanently deletes one session: its workspace accounting slot, its archive-set membership, and its durable log (through `SessionPersistence.delete`). A session still bound to the live SessionStore rejects with `WorkspaceLiveSessionError` — the caller stops a live agent first. Unknown ids are an idempotent no-op returning `false`; a removed record returns `true`. The deleted identity also leaves the registry's header index, so a later archive of the same id cannot succeed against a ghost.
...
- Destructive folder removal is a separate, absent capability; Workspace registration deletion never substitutes for it ([decision](../../../.agents/notes/implemented/feature/2026-07-27-workspace-registration-deletion.md)). Session deletion exists through `deleteSession` and lives at the identity level: the durable log is erased, so a deleted session cannot be restored.
