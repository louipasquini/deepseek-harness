...
- **Deletion removes the log and its session directory, best-effort on the directory** — `SessionPersistence.delete` unlinks the physical log; the per-session directory sweep is best-effort (an opposite-encoding artifact or temp may remain), and a torn removal leaves the artifact for the next scan, so deletion is retryable rather than crash-atomic.
