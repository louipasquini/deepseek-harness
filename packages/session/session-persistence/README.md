...
| `delete(id, signal?): Promise<boolean>` | Permanently remove one session: its durable log, metadata, and the coordinator's per-session bookkeeping. The explicit exception to the append-only contract — an administrative removal, never part of the write path. The session must not be live in the SessionStore (the caller stops a live agent first); any in-flight retirement of the identity settles before the removal. Unknown ids are an idempotent no-op returning `false`; a removed artifact returns `true`. |
...
| `deleteStored(id, signal?)` | Permanently remove one session's durable artifact. Invoked only by the coordinator's `delete` after the identity is neither live nor retiring. A torn removal may leave the artifact recoverable on the next scan — deletion is retryable and never corrupts other sessions. Returns `true` when an artifact was removed, `false` for an unknown id. |
...
- **Deletion is administrative, not lifecycle-driven** — `delete` permanently erases the durable log on explicit request; there is no retention policy, soft-delete tombstone, or automatic pruning ([session deletion note](../../../.agents/notes/implemented/feature/2026-08-24-session-deletion.md)).
